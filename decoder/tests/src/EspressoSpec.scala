...
object EspressoSpec extends TestSuite:
  val tests = Tests {
    "Espresso minimization" - {
      "should minimize a simple truth table" - {
      ...
      "should minimize a more complex truth table" - {
        val encoding = SortedMap(
      ...
      "should handle don't care patterns" - {
        val encoding = SortedMap(
      ...
      "should handle multiple truth tables" - {
      ...
      "should handle truth tables with all default values" - {
        val encoding = SortedMap(
      ...
      "should handle single input/output bit" - {
        val encoding = SortedMap(
          "ON"  -> BitSet.bitpat("1"),
          "OFF" -> BitSet.bitpat("0")
        )
      ...
      "should preserve output width with mixed defaults" - {
        val encoding = SortedMap(
          "A"       -> BitSet.bitpat("10"),
          "B"       -> BitSet.bitpat("01"),
      ...
      "should handle empty input sequences" - {
        intercept[IllegalArgumentException] {
          espresso(Seq.empty)
        }
      }

      "should minimize redundant entries" - {
        val encoding = SortedMap(
          "HIGH" -> BitSet.bitpat("1"),
          "LOW"  -> BitSet.bitpat("0")
        )

        // Create redundant patterns that should be minimized
        val pairs = Seq(
        ...
        // After minimization, should have fewer terms than original
        // (the three HIGH cases should be minimized)
        assert(result.table.size <= pairs.size)
      }

      "should handle multi-bit outputs correctly" - {
        val encoding = SortedMap(
      ...
      "should handle complex multi-term BitSets" - {
        val encoding = SortedMap(
      ...
      "should work with different default types" - {
      ...
      "result should be functionally equivalent to input" - {
        val encoding = SortedMap(
          "A"       -> BitSet.bitpat("01"),
          "B"       -> BitSet.bitpat("10"),
          "C"       -> BitSet.bitpat("11"),
      ...
      "should handle mixed default types that require splitting" - {
        // Default with mixed values (10)
        val encoding1 = SortedMap(
          "ON"      -> BitSet.bitpat("11"),
          "default" -> BitSet.bitpat("10")
        )

        val table1  = TruthTable("mixed_default_10", Seq((BitSet.bitpat("01"), "ON")), "default", encoding1)
        val result1 = espresso(Seq(table1))

        assert(result1.inputWidth == 2)
        assert(result1.outputWidth == 2)
        assert(result1.default == BitSet.bitpat("10"))

        // Default with mixed values (01)
        val encoding2 = SortedMap(
          "ON"      -> BitSet.bitpat("11"),
          "default" -> BitSet.bitpat("01")
        )

        val table2  = TruthTable("mixed_default_01", Seq((BitSet.bitpat("10"), "ON")), "default", encoding2)
        val result2 = espresso(Seq(table2))

        assert(result2.inputWidth == 2)
        assert(result2.outputWidth == 2)
        assert(result2.default == BitSet.bitpat("01"))

        // Test combined tables with mixed defaults
        val result3 = espresso(Seq(table1, table2))

        assert(result3.inputWidth == 2)
        assert(result3.outputWidth == 4)
        assert(result3.default == BitSet.bitpat("1001")) // Combined defaults

        // Create a lookup function for the result
        def lookup(input: BitPat): BitPat = {
          result3.table.find(_._1 == input).map(_._2).getOrElse(result3.default)
        }

        // Test combined results preserve original mappings
        assert(lookup(BitSet.bitpat("01")) == BitSet.bitpat("1101"))
        assert(lookup(BitSet.bitpat("10")) == BitSet.bitpat("1011"))
      }

      "should handle complex mixed default with zeros, ones and don't cares" - {
        ...
        val truthTable = TruthTable("complex_mixed_default", pairs, "default", encoding)
        val result     = espresso(Seq(truthTable))

        assert(result.inputWidth == 3)
        assert(result.outputWidth == 6)
        assert(result.default == BitSet.bitpat("10?1?0"))
        ...
        // Test original mappings are preserved
        assert(lookup(BitSet.bitpat("001")) == BitSet.bitpat("000000"))
        assert(lookup(BitSet.bitpat("110")) == BitSet.bitpat("111111"))

        // Test unmapped inputs return default
        assert(lookup(BitSet.bitpat("000")) == BitSet.bitpat("10?1?0"))
        assert(lookup(BitSet.bitpat("111")) == BitSet.bitpat("10?1?0"))
      }

      "should handle multiple truth tables with mixed defaults" - {
        // First table with mixed default (10)
        val encoding1 = SortedMap(
          "A"       -> BitSet.bitpat("01"),
          "default" -> BitSet.bitpat("10")
        )

        // Second table with mixed default (01?)
        val encoding2 = SortedMap(
          "B"       -> BitSet.bitpat("110"),
          "default" -> BitSet.bitpat("01?")
        )

        val table1 = TruthTable("t1_mixed", Seq((BitSet.bitpat("10"), "A")), "default", encoding1)
        val table2 = TruthTable("t2_mixed", Seq((BitSet.bitpat("01"), "B")), "default", encoding2)

        val result = espresso(Seq(table1, table2))

        assert(result.inputWidth == 2)
        assert(result.outputWidth == 5)                  // 2 + 3
        assert(result.default == BitSet.bitpat("1001?")) // Combined defaults
        ...
        // Test combined results preserve original mappings
        assert(lookup(BitSet.bitpat("10")) == BitSet.bitpat("0101?"))
        assert(lookup(BitSet.bitpat("01")) == BitSet.bitpat("10110"))

        // Test default is used for unmapped inputs
        assert(lookup(BitSet.bitpat("00")) == BitSet.bitpat("1001?"))
        assert(lookup(BitSet.bitpat("11")) == BitSet.bitpat("1001?"))
      }
    }
  }
