...
sealed case class FirrtlType(
  ...
  def toMLIR(
    ...
    case s if s == "uint"       => handler.firrtlTypeGetUInt(width.get)
    case s if s == "sint"       => handler.firrtlTypeGetSInt(width.get)
    case s if s == "analog"     => handler.firrtlTypeGetAnalog(width.get)
    case s if s == "bundle"     =>
...
class UInt(width: Int, const: Boolean) extends FirrtlType("uint", const, Some(width), None, None, None)

class SInt(width: Int, const: Boolean) extends FirrtlType("sint", const, Some(width), None, None, None)

class Analog(width: Int, const: Boolean) extends FirrtlType("analog", const, Some(width), None, None, None)
