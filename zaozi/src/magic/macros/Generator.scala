  ...
  def transform(
    ...
    definition match
      case ClassDef(name, constr, parents, selfOpt, body)
        ...
        val interfaceDef = makeInterfaceDef("interface", tpiI)
        val probeDef     = makeInterfaceDef("probe", tpiP)

        val parseParameterDef = makeParseParameterDef
        val mainDef           = makeMainDef

        val newBody = makeMainDef :: parseParameterDef :: interfaceDef :: probeDef :: body
