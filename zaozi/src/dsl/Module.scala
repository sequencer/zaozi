...
object Module:
  def apply[P <: Parameter, I <: Interface[P]](
    ...
    given context: Context = new Context:
      ...
      val interfaceWire: Wire[I] = new Wire[I](s"io", NameKind.Droppable, interface)
      interfaceWire._refer = handler
        .OpBuilder("firrtl.wire", currentBlock, handler.unkLoc)
        .withNamedAttr("name", handler.mlirStringAttrGet(interfaceWire.name))
        .withNamedAttr("nameKind", handler.firrtlAttrGetNameKind(me.jiuyang.zaozi.internal.toMLIR(interfaceWire.nameKind)))
        .withNamedAttr("annotations", handler.emptyArrayAttr)
        .withResult(interfaceWire.tpe.firrtlType.copy(fields = interfaceWire.tpe.firrtlType.fields.map(_.map(f => f.copy(isFlip = !f.isFlip)))).toMLIR(handler))
        .build().results.head
      interfaceWire.tpe.elements.zipWithIndex.foreach:
        case (ele, idx) =>
          ...
          val wire = handler
            ...
            .withOperand(/* input */ interfaceWire._refer)
