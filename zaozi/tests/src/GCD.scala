...
case class SubtractorParameter(width: Int, moduleName: String, layers: Seq[Layer]) extends Parameter
...
given TokensReader.Simple[Layer] with
  def shortName: String = "layer"
  def read(strs: Seq[String]): Either[String, Layer] = Right(Layer(strs.head))
given upickle.default.ReadWriter[Layer]               = upickle.default.readwriter[String].bimap(_.name, Layer(_))
...
object GCD extends Generator[GCDParameter, GCDIO, GCDProbe]:
  def architecture(parameter: GCDParameter) =
    ...
    val sub = Subtractor.instantiate(new SubtractorParameter(x._tpe._width, "Sub", parameter.layers))
    sub.io.a := x
    sub.io.b := y
    val a = sub.io.z
