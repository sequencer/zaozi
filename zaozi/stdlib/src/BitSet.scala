...
object BitSet:
  ...
  def bitpat(str: String):                                BitPat      =
    require(str.forall(Seq('1', '0', '?').contains))
    bitpat(
      BigInt(str.replace('?', '0')),
      BigInt(str.replace('0', '1').replace("?", "0")),
      str.length
    )
  ...
  def apply(bs: Set[BitSet]):                             BitSet      = new BitSet:
    val terms: Set[BitPat] = bs.flatMap(_.terms)
  def range(
    _start:  BigInt,
    _length: BigInt
  ...
  def range(
    ...
    override val start:  BigInt = _start
    override val length: BigInt = _length
  ...
  def DC(width: Int = 1): BitPat = BitSet.bitpat("?" * width)
...
given upickle.default.ReadWriter[BitSet] = upickle.default
  ...
  .bimap[BitSet](
    bs => bs.toString,
    str => BitSet(str.split('|').map(BitSet.bitpat).sorted.toSet)
  )
...
trait RangeBitSet extends BitSet:
  ...
  val start:  BigInt
  val length: BigInt
  override def toString:      String      = s"0x${start.toString(16)}-0x${(start + length).toString(16)}"
  // validation
  ...
  final val terms:            Set[BitPat] =
    ...
    while (left > 0)
      ...
      collected.add(new BitPat:
        val value:          BigInt = ptr
        val mask:           BigInt = (BigInt(1) << out.width) - inc
        override val width: Int    = out.width
      )
      ptr += inc
      left -= inc
