    ...
    test("Z3 output"):
      test("Z3 demo with sat output"):
        val parsed = smtZ3Test(
          ...
          val x = smtValue(Bool)
          smtAssert(x)
        }.getSatResult ==> ("sat", true)

      test("Z3 demo with unsat output"):
        smtZ3Test(
          "(declare-const x Bool)",
          ...
          val x = smtValue(Bool)
          smtAssert(x & !x)
        }.getSatResult ==> ("unsat", false)

      test("Z3 demo with unknown output"):
