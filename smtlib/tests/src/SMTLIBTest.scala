...
extension (z3Output: Seq[(String, Boolean | Int)])
  def getSatResult: (String, Boolean | Int) =
    z3Output.head

  def getModel: Seq[(String, Boolean | Int)] =
    z3Output.head ==> ("sat", true)
    z3Output.tail
...
def smtZ3Test(checkLines: String*)(body: (Arena, Context, Block) ?=> Unit): Seq[(String, Boolean | Int)] =
  given Arena   = Arena.ofConfined()
  given Context = summon[ContextApi].contextCreate
  summon[SmtDialect].loadDialect()
  summon[FuncDialect].loadDialect()
